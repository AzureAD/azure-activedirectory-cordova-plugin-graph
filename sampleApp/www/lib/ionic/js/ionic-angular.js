...
/**
 * @ngdoc method
 * @name $ionicConfigProvider#views.maxPrefetch
 * @description Sets the maximum number of views to prefetch from the templateUrls defined in
 ...
 * @usage
 * State views are cached automatically, but you can optionally cache other views.
 *
 * ```js
 ...
 *     $ionicConfigProvider.views.prefetch(false);
 *
 ...
 *         templateUrl: "tabs-views/tabs.html"
 *       })
 *       .state('tabs.home', {
 *         url: "/home",
 *         views: {
 *           'home-tab': {
 *             prefetchTemplate: false,
 *             templateUrl: "tabs-views/home.html",
...
function($http, $templateCache, $timeout) {
  ...
  function run() {
    ...
    while (i < 4 && (template = toCache.pop())) {
      // note that inline views are ignored by this request
      if (isString(template)) $http.get(template, { cache: $templateCache });
      i++;
    }
    // only preload 3 views a second
    if (toCache.length) {
      $timeout(run, 1000);
    }
  }
 ...
 *
 * The ionNavView directive is used to render views in your application. Each template
 ...
 *
 * Pages are loaded by the URLs given. One simple way to create views in Angular is to put
